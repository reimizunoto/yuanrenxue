...
def get_data(page):
    ...
    cookies = {
        ...
        "Hm_lpvt_9bcbda9cbf86757998a2339a0437208e": "1711523334",
        "Hm_lpvt_c99546cf032aaa5a679230de9a95c7db": "1711523339"
    }
...
def display(key: str, val: str):
    b_key = base64.b64encode((key + val).encode()).decode().replace("=", "")
    j_key = MD5.new(b_key.encode())
    return j_key.hexdigest()


def img_sort(data : list):
    i = 1
    for item in data:
        item[1] = float(item[1]) + 11.5 * i
        i += 1
...
def info_to_numbers(data):
    ...
    td_tag = re.findall("<td>(.*?)</td>", data["info"])  # 获取所有td标签
    img_data = []
    for td in td_tag:
        img_data.append(re.findall("<img (.*?)>", td))  # 获取所有img标签

    dis_null = display(data["key"], data["value"])
    result = []
    for img in img_data:
        temp = []
        for item in img:
            if dis_null in item:
                continue
            src = re.findall("src=\"(.*?)\"", item)
            src[0] = img_to_number[src[0]]
            px = re.findall(".*?style=\"left:(.*?)px\"", item)
            temp.append([src[0], px[0]])
        result.append(img_sort(temp))
    print(result)
    return sum(result)
...
if __name__ == '__main__':
    ...
    for page in range(1, 6):
        data = get_data(page)
        res.append(info_to_numbers(data))
