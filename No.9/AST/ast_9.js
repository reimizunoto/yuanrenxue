...
// 读取路径和保存路径
readPath = 'No.9\\AST\\9.js'
writePath = 'No.9\\AST\\9_decode.js'
...
traverse(ast, {
    VariableDeclarator: {
        enter(path) {
            let { node } = path;
            if (!(node.init && node.init.properties && t.isObjectExpression(node.init))) return; // 过滤不满足要求的内容
            // 获取指令的对象名
...
traverse(ast, {
    VariableDeclarator: {
        enter(path) {
            let { node, scope } = path;
            if (!(node.init && node.init.properties && t.isObjectExpression(node.init))) return;
